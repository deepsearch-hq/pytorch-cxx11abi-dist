...
mkdir -p $WORKING_DIR
pushd $WORKING_DIR
git clone --depth 1 -b v2.1.0 https://github.com/pytorch/pytorch.git \
    && cd pytorch \
    && sed -i "" '/cast-function-type/d' CMakeLists.txt \
    && git submodule sync \
    && git submodule update --init --recursive --jobs `nproc --all` \
    && CFLAGS=$CFLAGS CXXFLAGS=$CXXFLAGS PYTORCH_BUILD_VERSION=2.1.0 \
       USE_PYTORCH_QNNPACK=0 USE_NNPACK=0 USE_QNNPACK=0 USE_MPS=0 \
       USE_XNNPACK=0 PYTORCH_BUILD_NUMBER=1 python3 setup.py bdist_wheel \
    && cd ..

mv ./pytorch/dist/torch-2.1.0-cp311-cp311-macosx_*.whl ../
popd
rm -rf $WORKING_DIR
